...
<body>
	...
	<div data-role="page" id="#pageone">
		...
		<!-- 地图 -->
		<div  id="map" style="padding: 5px 20px;margin-bottom: 5px;"></div>
		<script type="text/javascript"
			src="http://maps.google.cn/maps/api/js?sensor=false&key=AIzaSyAeEMBoxHAF19fSyuNGWC80yQPX9urbp44"></script>

		<!-- 底部导航 -->
		<div data-role="footer" id="foot">
			<div data-role="controlgroup" data-type="horizontal" class="divcss5">

				<a href="#panel-right" data-role="button" id='history_search_tv'></a>
				 <a onclick="turn_page()" data-role="button" id='lc_tv'></a> 
				<input	id="playbtn" type="button" onclick="playbtn()" /> 
				<select
					id="xila" class="ci">
				...
				<a onclick="real_refresh()" data-role="button" id='real_refresh'>实时刷新</a>
...
<script type="text/javascript">
	...
	document.getElementById("lc_tv").innerText = lc_tv[lsel];
	...
	function carselchange(type) {
		...
		$.get("../ToolsServlet?action=changeSelect&weid="
				+ GetQueryString("weid") + "&did=" + $("#did").val(), function(
				data, status) {
		});
		...
		for (var i = 0; i < car_marker.length; i++)
			if (car_marker[i] != null)
				//car_marker[i].setIcon('images/carundefined.gif');
					{
				var imgurl;
				if(Date.parse(new Date())/1000-objs[i].time>7200)
					imgurl='images/caroff.gif';
				else if(Date.parse(new Date())/1000-objs[i].time>3600&&Date.parse(new Date())/1000-objs[i].time<7200)
					imgurl='images/carstop.gif';
				else
					imgurl='images/carundefined.gif';
				car_marker[i].setIcon(imgurl);
				}
		//设置选中图片
		if (car_marker[$("#did ").get(0).selectedIndex] != null) {
			...
			if (objs.length > 0) {
				...
				$("#carstates").append(
						"<li>" + objs[$("#did ").get(0).selectedIndex].lasttime
								+ objs[$("#did ").get(0).selectedIndex].status
								+ "</li>").trigger("create");
				/* 	$("#carstates").append(
							"<li>" + 
									+ "</li>").trigger("create"); */
				$("#carstates").append("<li>" + "平台服务结束日期" + "</li>").trigger(
						"create");
				$("#carstates")
						.append(
								"<li>"
										+ objs[$("#did ").get(0).selectedIndex].service_tinme
										+ "</li>").trigger("create");
			...
			$("#stop_list").append("<li>" + null_msg_tv[lsel] + "</li>")
					.trigger("create");
			//$("#stop_list").listview("refresh");

			$("#over_list").append("<li>" + null_msg_tv[lsel] + "</li>")
					.trigger("create");
			...
			if (strs != "undefined" && strs.length > 0
				...
				if(strs[3]!=null)
				{
				document.getElementById("round_time_tv").innerHTML = round_start_tv[lsel]
						+ "<br/>"
						+ strs[3].split("-")[0]
						+ "<br/>"
						+ round_end_tv[lsel] + "<br/>" + strs[3].split("-")[1];
				}
	...
	function initLeftPanel(objs) {
		if (objs.length > 0)
			...
			$("#did").append(
					"<option value='0'>" + nolock_tip_tv[lsel] + "</option>")
					.trigger("create");
			$("#did2").append(
					"<option value='0'>" + nolock_tip_tv[lsel] + "</option>")
					.trigger("create");
	...
	function initRightPanel() {
		...
		for (var i = 0; i <= 6; i++)
			$("#day_sel").append(
					"<option value='"+i+"'>" + i + day_tv[lsel] + "</option>")
					.trigger("create");
		//初始化时
		for (var i = 0; i <= 23; i++)
			$("#hours_sel").append(
					"<option value='"+i+"'>" + i + hour_tv[lsel] + "</option>")
					.trigger("create");
	...
	function createMarker(i, t) {
		...
		//判断电压
		if (objs[i].type == 4) {
			bat_str = "<b>" + bat_tv[lsel] + ":</b>" + objs[i].bat + "<b>"
					+ elec_tv[lsel] + ":</b>" + objs[i].elec;
		...
		geocoder.geocode({
			...
			if (responses && responses.length > 0) {
				...
				infowindow_text[i] = String.format(html, objs[i].no,
						objs[i].name, time_temp, objs[i].speed,
						place_nofind_tv[lsel], bat_str, lead_btn);
			}
	...
	function drawline() {
		...
		for (var i = 1; i < pointObjs[0].length; i++) {
			var tempLine = [
					new google.maps.LatLng(pointObjs[0][i - 1].lat,
							pointObjs[0][i - 1].lng),
					new google.maps.LatLng(pointObjs[0][i].lat,
							pointObjs[0][i].lng) ];

			//轨迹颜色
			if (pointObjs[0][i].overSpeed > 0
					&& pointObjs[0][i - 1].overSpeed > 0) {
				linecor = "#FF0000";
				//超速点
		...
		for (var i = 0; i < pointObjs[1].length; i++) {
			...
			stop_marker[stop_marker.length] = new google.maps.Marker({
				...
				icon : stop_image,
				shape : shape,
				title : stop_point_tv[lsel]
						+ "<br>"
						+ new Date(pointObjs[1][i].time * 1000)
								.format("yyyy年MM月dd日 hh:mm:ss") + "<br>"
						+ stop_time_tv[lsel] + ":" + day + day_tv[lsel] + hour
						+ hour_tv[lsel] + min + min_tv[lsel] + afterMin
						+ second_tv[lsel],
				cursor : "" + total / 60,
				zIndex : i
			});
	...
	function searchbtn() {
		...
		$.get("../carLocationServlet?action=getHistroy&didhis="
			...
			if (pointObjs == null) {
				...
				if (pointObjs[0].length > 5000) {
					var se = confirm("所选时间段内点数过多，为您优化数据");
					if (se == true) {
						pointObjs.length = 5000;
						drawline();
					} else {
						drawline();
					}
	...
	function resetMkPoint(i) {
		if (i < pointObjs[0].length - 1) {
			caraction = setTimeout(function() {
				i++;
				car_marker[$("#did ").get(0).selectedIndex]
						.setPosition(new google.maps.LatLng(
								pointObjs[0][i].lat, pointObjs[0][i].lng));

				car_marker[$("#did ").get(0).selectedIndex]
						.setIcon('images/car' + pointObjs[0][i].dir + '.gif');
	...
	function turn_page() {
		...
		var real_refresh_action;
		var isreal = false;
	function real_refresh() {
		if(!isreal)
		{
			popup("已开启实时刷新");
			isreal=true;
	...
	//移动轨迹点
		real_refresh_action = setInterval(function() {
		
		
				$.get("../carLocationServlet?action=getCarNow&car_id="
					+ $("#did ").get(0).value, function(data, status) {
					var gpsmsg = eval(data);
					...
					//判断电压
					if (gpsmsg[0].type == 4) {
						bat_str = "<b>" + bat_tv[lsel] + ":</b>" + gpsmsg[0].bat + "<b>"
								+ elec_tv[lsel] + ":</b>" + gpsmsg[0].elec;
						/* lead_btn="<select id='loc_time'><option value='360'>30分钟</option><option value='720'>1小时</option><option value='4320'>6小时</option><option value='8640'>12小时</option></select><button style='margin-left: 20px;padding: 10px' id='loc_btn"+i+"' onclick='loc_device("+objs[i].gpsPosiLat+","+objs[i].gpsPosiLon+","+i+")'>开始追踪</button>" */
						lead_btn = "<button style='margin-left: 20px;padding: 10px' id='loc_btn"
								+ i
								+ "' onclick='goloc()'>"
					...
					var time_temp = new Date(gpsmsg[0].time * 1000).format("MM/dd hh:mm");
					
					
					var latLng = new google.maps.LatLng(Number(gpsmsg[0].gpsPosiLat),Number(gpsmsg[0].gpsPosiLon));
					
					var temp_infowindow_text;
					
					geocoder.geocode({
						latLng : latLng
					}, function(responses) {
						if (responses && responses.length > 0) {
							temp_infowindow_text = String.format(html, gpsmsg[0].no,
									gpsmsg[0].name, time_temp, gpsmsg[0].speed,
									responses[0].formatted_address, bat_str, lead_btn);
						} else {
							temp_infowindow_text = String.format(html, gpsmsg[0].no,
									gpsmsg[0].name, time_temp, gpsmsg[0].speed,
					...
					infowindow.setContent(temp_infowindow_text);
					infowindow.open(map, car_marker[$("#did ").get(0).selectedIndex]);	
						
					
					car_marker[$("#did ").get(0).selectedIndex]
						.setPosition(new google.maps.LatLng(
								gpsmsg[0].gpsPosiLat, gpsmsg[0].gpsPosiLon));
		...
		{
			popup("已关闭实时刷新");
			clearInterval(real_refresh_action);
		}
