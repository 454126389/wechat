...
(function() {
	...
	HTMLElement.prototype.touchScroll = function() {
		...
		document
				.addEventListener("touchmove",
						function(e) {
							if (isstart) {
								var newY = getClient(e), distance = startY
										- newY, time = new Date().getTime()
										- startTime;
	...
	/*
	 * 以一定的速度滚动一段距离，在某个减速度的影响下 参数： speed -- 初速度(ms)
	 */

	HTMLElement.prototype.speedTo = function(speed) {
		var criticalSpeed = 0.2;// 可以引发滚动的临界速度
		if (speed && Math.abs(speed) > criticalSpeed) {
			var self = this, a = 1 / 200, // 减速度
			reverse = speed < 0, // 是否反向(向上为正)
			a = reverse ? -a : a, oriSpeed = speed, // 初速度
			t = 50, // 两帧之间的时间间隔(ms)

			ori = self.scrollTop, d = speed * t, iv = setInterval(function() {
				speed -= t * a
				d += (speed * t);
