...
public class ConnectionPoolDao {
	...
	// //-conqueror_app
	// 获取绑定车辆数量
	// public static com.ruiyi.wechat.po.User getUser(String wechatid) {
	// // TODO Auto-generated method stub
	//
	// com.ruiyi.wechat.po.User user;
	//
	...
	//
	// String sql = "SELECT * FROM terminal_wechat WHERE WECHAT_ID='"
	// + wechatid + "'";
	// String num = null;
	// try {
	...
	public static int getUserParmaterNum(String uid) {
		...
		String sql = "SELECT count(*) as n from user_parameter where USER_ID=?";
	...
	public static List<SettingInfo> getLockCarSetting(String wechatid) {
		...
		try {
			...
			while (rs.next()) {
				...
				/*
				 * STZS = sTZS; CGPS = ; MPHN = mPHN; FPHN = fPHN; REST = rEST;
				 */

				SettingInfo car = new SettingInfo(rs.getString("ID"),
						rs.getString("ALIAS"), rs.getString("FORTIRY"),
						rs.getString("SGPS"), rs.getString("TR_T"),
						rs.getString("EMAIL_OPEN"), rs.getInt("WIFI"), "",
						rs.getString("SOS_OPEN"), rs.getString("STZS"),
						rs.getString("CGPS"), rs.getString("MPHN"),
						rs.getString("FPHN"), rs.getString("REST"),
						rs.getString("VATIME"), rs.getString("LSEN"));
	...
	public static List<CarInfo> getLockCarList(String wechatid) {
		...
		connPool = ConnectionPool.getPool();
		// String sql =
		// "SELECT p.ID,p.ALIAS FROM parameter as p,terminal_wechat as t WHERE t.ID=p.ID AND t.WECHAT_ID ='"
		// + wechatid + "'";

		String sql = "SELECT p.ID,p.ALIAS FROM parameter as p,terminal_wechat as t WHERE t.ID=p.ID AND t.WECHAT_ID =?";
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				CarInfo car = new CarInfo(rs.getString("ID"),
						rs.getString("ALIAS"));
				carlist.add(car);
			}
		} catch (Exception ex) {
	...
	public static List<CarInfo> getCarListOnMap(String wechatid) {
		...
		PreparedStatement ps = null;
		connPool = ConnectionPool.getPool();
		String sql = "SELECT p.ID,p.CHOICE,p.ALIAS,p.FENCE FROM parameter as p,terminal_wechat as t WHERE t.ID=p.ID AND t.WECHAT_ID ='"
				+ wechatid + "'";
		/*
		 * String sql =
		 * "SELECT ID,ALIAS FROM parameter WHERE WECHAT_ID='"+wechatid
		 * +"' union select ID,ALIAS from hsjfrz where wechat_id ='"
		 * +wechatid+"'";
		 */
		...
		try {
			...
			while (rs.next()) {
				CarInfo car = new CarInfo(rs.getString("ID"),
						rs.getString("CHOICE"), rs.getString("ALIAS"),
						rs.getString("FENCE"));
	 ...
	 * 
	 * ConnectionPool connPool = null; Connection conn = null; PreparedStatement
	 * ps = null; connPool = ConnectionPool.getPool();
	 * 
	 * Boolean isTrue = false;
	 * 
	 * 
	 * String sql
	 * ="UPDATE parameter SET CHOICE =0 WHERE WECHAT_ID='"+wechatid+"'"; int rs;
	 * try { conn = connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeUpdate(); conn.commit();
	 * 
	 * System.out.println(rs);
	 * 
	 * isTrue = true; } catch (Exception ex) { isTrue = false;
	 * System.out.println("绑定车辆失败：" + ex.getMessage()); } finally {
	 * connPool.closeConnection(ps, conn); } return isTrue; }
	 * 
	 ...
	 * 
	 * 
	 * String sql ="UPDATE parameter SET CHOICE =1 WHERE ID="+id; int rs; try {
	 * conn = connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeUpdate(); conn.commit();
	 * 
	 * System.out.println(rs);
	...
	public static List<CarInfo> getCarInfo(String did) {
		...
		String sql = "SELECT ID,ALIAS,FENCE FROM parameter WHERE ID='" + did
				+ "'";
		/*
		 * String sql =
		 * "SELECT ID,ALIAS FROM parameter WHERE WECHAT_ID='"+wechatid
		...
		try {
			...
			while (rs.next()) {
				CarInfo car = new CarInfo(rs.getString("ID"),
						rs.getString("ALIAS"), rs.getString("FENCE"));
				carlist.add(car);
			}
		} catch (Exception ex) {
			System.out.println("获取用户绑定车辆列表失败：" + ex.getMessage());
	 ...
	 * wechatid + "'"; String sql =
	 * "select ID,ALIAS,FENCE from hsjfrz where wechat_id ='"+wechatid+"'";
	 * 
	 * 
	 * 
	 * 
	 * 
	 * List<CarInfo> carlist = new ArrayList<CarInfo>(); ResultSet rs = null;
	 * try { conn = connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeQuery(); while (rs.next()) { CarInfo car = new
	 * CarInfo(rs.getString("ID"), rs
	 * .getString("ALIAS"),rs.getString("FENCE")); carlist.add(car); } } catch
	 * (Exception ex) { System.out.println("获取用户绑定车辆列表失败：" + ex.getMessage()); }
	 * finally { connPool.closeConnection(rs, ps, conn); } return carlist; }
	 */
	...
	// 获得wechat用户列表
	/*
	 * public static List<String> getWechatUserList() { ConnectionPool connPool
	 * = null; Connection conn = null; PreparedStatement ps = null; connPool =
	 * ConnectionPool.getPool();
	 * 
	 * 
	 * 
	 * String sql =
	 * "SELECT distinct WECHAT_ID FROM parameter WHERE WECHAT_ID is not null";
	 * 
	 * List<String> userList = new ArrayList<String>(); ResultSet rs = null; try
	 * { conn = connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeQuery(); while (rs.next()) {
	 * userList.add(rs.getString("WECHAT_ID")); } } catch (Exception ex) {
	 * System.out.println("获得wechat用户列表失败：" + ex.getMessage()); } finally {
	 * connPool.closeConnection(rs, ps, conn); } return userList; }
	 */

	// 获得wechat用户列表
	/*
	 * public static List<CarIdWeid> getWechatList() { ConnectionPool connPool =
	 * null; Connection conn = null; PreparedStatement ps = null; connPool =
	 * ConnectionPool.getPool();
	 * 
	 * String sql =
	 * "SELECT ID,WECHAT_ID FROM  parameter WHERE LENGTH(WECHAT_ID)>1";
	 * 
	 * List<CarIdWeid> userList = new ArrayList<CarIdWeid>(); ResultSet rs =
	 * null; try { conn = connPool.getConnection(); ps =
	 * conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeQuery(); while (rs.next()) { CarIdWeid car = new
	 * CarIdWeid(rs.getString("ID"), rs .getString("WECHAT_ID"));
	 * userList.add(car); } } catch (Exception ex) {
	 * System.out.println("获得wechat用户列表失败：" + ex.getMessage()); } finally {
	 * connPool.closeConnection(rs, ps, conn); } return userList; }
	 */
	...
	public static List<CarInfo> getUnLockCarList(String phone) {
		...
		String sql = "SELECT * FROM parameter as p where p.ZH_NO=? and p.ID not in(SELECT t.ID from terminal_wechat as t)";
	...
	public static String[] getPidByUid(String uid) {
		...
		String sql = "SELECT * FROM user_parameter WHERE USER_ID=?";
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				// roundrail.add(rs.getString("PARAMETER_ID"));
				// roundrail[roundrail.length]=rs.getString("PARAMETER_ID");
				roundrail = insert(roundrail, rs.getString("PARAMETER_ID"));
			}
	...
	public static Boolean doLockCar(String wechatid, String[] carnolist) {
		...
		/*
		 * String sql = "insert into terminal_wechat ()value SET WECHAT_ID = '"
		 * + wechatid + "' WHERE " + nolist.substring(0, nolist.length() - 4);
		 */

		try {
			String sql = "INSERT INTO terminal_wechat(ID,WECHAT_ID)VALUES(?,?)";
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			for (int i = 0; i < carnolist.length; i++) {
				ps.setLong(1, Long.parseLong(carnolist[i]));
	...
	// 绑定车辆：
	public static void doUnlockCar(String pid) {
		...
		try {
			String sql = "DELETE FROM terminal_wechat WHERE ID=?";
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			ps.setString(1, pid);
			ps.addBatch();
	...
	public static Boolean doLockCar(String wechatid, String pid) {
		...
		//解绑其他绑定者
		ConnectionPoolDao.doUnlockCar(pid);
	...
	// 解绑车辆：
	public static Boolean doUnLockCar(String[] carnolist, String weid) {
		...
		/*
		 * StringBuffer nolist = new StringBuffer(); for (int i = 0; i <
		 * carnolist.length; i++) { nolist.append("ID=" + carnolist[i] +
		 * " OR "); }
		 * 
		 * if (nolist.length() == 0) { isSuccese = false; }
		 */

		String sql = "DELETE FROM terminal_wechat  WHERE ID =? AND WECHAT_ID=?";
	 ...
	 * Boolean isTrue = false;
	 * 
	 * String sql = "UPDATE parameter m set m.WECHAT_ID = '"+wechatid+
	 * "' WHERE m.ID IN(SELECT temp.PARAMETER_ID FROM (SELECT * FROM user_parameter WHERE USER_ID='"
	 * +pid+"') temp)";
	 * 
	 * int rs; try { conn = connPool.getConnection(); ps =
	 * conn.prepareStatement(sql);
	 * 
	 * rs = ps.executeUpdate(); conn.commit();
	 * 
	 ...
	 * 
	 * isTrue = true; } catch (Exception ex) { isTrue = false;
	 * System.out.println("一键绑定车辆：" + ex.getMessage()); } finally {
	 * connPool.closeConnection(ps, conn); } return isTrue; }
	 */
	 ...
	 * 
	 * String sql =
	 * "UPDATE parameter m set m.WECHAT_ID = NULL WHERE m.ID IN(SELECT temp.PARAMETER_ID FROM (SELECT * FROM user_parameter WHERE USER_ID='"
	 * +pid+"') temp)";
	 * 
	 * int rs; try { conn = connPool.getConnection(); ps =
	 * conn.prepareStatement(sql);
	...
	// 改变设备设置定位器
	public static Boolean updatDeviceSetting2(String id, String stzs,
			String cpgps, String mphnlist, String fphnlist, String restswch,
			String vatime, String lsen) {
		...
		Boolean isSucc = false;
		String sql = "UPDATE parameter SET STZS = ? , CGPS = ? , MPHN= ?,FPHN=? ,REST=?,VATIME=?,LSEN=? WHERE ID='"
				+ id + "'";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			...
			ps.setInt(7, Integer.parseInt(lsen));
			ps.executeUpdate();
			conn.commit();
			isSucc = true;
		} catch (Exception ex) {
	...
	public static List<Voiture> getCarTypeList() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Voiture cartype = new Voiture(rs.getString("ID"),
						rs.getString("TYPE"));
				carlist.add(cartype);
			}
		} catch (Exception ex) {
	...
	public static RegisterDevice RegisterDeviceSelect(String id) {
		...
		String sql = "select pt.CUT,LANGUAGE,ZH_NO,F_NO,ALIAS,FENCE,EMAIL_OPEN,SMS_OPEN,FORTIRY,SGPS,RMAP,TR_T,SPRU,NEWSOPEN,VATIME from parameter as p,product as pt where pt.ID=LEFT(p.ID,3) and p.ID="
				+ id;
		ResultSet rs = null;
		Boolean isTrue = false;
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				registerDevice = new RegisterDevice(rs.getString("CUT"),
						rs.getString("LANGUAGE"), rs.getString("ZH_NO"),
						rs.getString("F_NO"), rs.getString("ALIAS"),
						rs.getString("FENCE"), rs.getString("EMAIL_OPEN"),
						rs.getString("SMS_OPEN"), rs.getString("FORTIRY"),
						rs.getString("SGPS"), rs.getString("RMAP"),
						rs.getString("TR_T"), rs.getString("SPRU"),
						rs.getString("NEWSOPEN"), rs.getString("VATIME"));
			}
	...
	public static String getBatchId(String pid) {
		...
		String sql = "SELECT BATCH_ID FROM terminal where ID= ?";
		String batchid = null;
		ResultSet rs = null;
		try {
			conn = connPool.getConnection();
	...
	public static Batch getBatch(String bachtid) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				batchid = new Batch(rs.getString("ID"),
						rs.getString("TIME_ZONE"), rs.getString("VALIDITY"),
						rs.getString("TIME"), rs.getString("NUM"),
						rs.getString("LANGUAGE"), rs.getString("AGENT_ID"),
	...
	public static Boolean addParameter(Parameter parameter) {
		...
		String sql = "INSERT INTO parameter (ID,LANGUAGE,ZH_NO,ALIAS,PRODUCT_ID,VOITURE_ID) VALUES (?,?,?,?,?,?)";
		ResultSet rs = null;
		Boolean isTrue = false;
		try {
			conn = connPool.getConnection();
	...
	public static Boolean addParameter2(Parameter parameter) {
		...
		String sql = "INSERT INTO parameter (ID,PRODUCT_ID,ALIAS) VALUES (?,?,?)";
		ResultSet rs = null;
		Boolean isTrue = false;
		try {
			conn = connPool.getConnection();
	 ...
	 * ConnectionPool connPool = null; Connection conn = null; PreparedStatement
	 * ps = null; connPool = ConnectionPool.getPool(); String sql =
	 * "UPDATE parameter SET PRODUCT_ID=?,WECHAT_ID=?,ALIAS=? WHERE ID=?";
	 * ResultSet rs = null; Boolean isTrue = false; try { conn =
	 * connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * 
	 ...
	 * 
	 * ps.executeUpdate(); conn.commit(); isTrue = true; } catch (Exception ex)
	 * { isTrue = false; System.out.println("添加设备失败：" + ex.getMessage()); }
	 * finally { connPool.closeConnection(rs, ps, conn); } return isTrue; }
	 */
	...
	// 添加service状态表
	public static Boolean addServiceParameter(String PARAMETER_ID,
			String sdate, String edate, String edate2) {
		ConnectionPool connPool = null;
		Connection conn = null;
		PreparedStatement ps = null;
		connPool = ConnectionPool.getPool();

		String sql = "INSERT INTO service_parameter (SERVICE_ID,STATE,STARTTIME,ENDTIME,PARAMETER_ID)VALUES "
				+ "(1,1,NULL,NULL,?)"
				+ ",(3,1,NULL,NULL,?)"
				+ ",(4,1,?,?,?),"
				+ "(5,1,NULL,NULL,?)," + "(6,1,?,?,?)," + "(7,1,NULL,NULL,?)";
		ResultSet rs = null;
		Boolean isTrue = false;
		try {
			conn = connPool.getConnection();
	...
	// 改变registrationId
	public static Boolean addTerminalRegistrationId(String id,
			String registrationId, String imei) {
		...
		Boolean isSucc = false;
		String sql = "UPDATE terminal SET IMEI = ?,REGISTRATION_ID= ? WHERE ID='"
				+ id + "'";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
		...
		try {
			...
			while (rs.next()) {
				user = new User(rs.getString("USERNAME"), "", "",
						...
						rs.getFloat("BALANCE"), "", rs.getString("QQ"),
						rs.getString("ID"));
			}
	...
	// 改变用户信息
	public static Boolean updataUserInfo(String id, String tel, String qq,
			String email) {
	...
	public static Boolean updataParmterPhone(String id, String tel) {
		...
		Boolean isSucc = false;
		String sql = "UPDATE parameter SET ZH_NO = ?  WHERE ID ='" + id + "'";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 获得用户的所有设备ID
	public static List<Car> getUserParamterById(String uid) {
		...
		String sql = "SELECT * FROM parameter WHERE ID in(SELECT PARAMETER_ID FROM user_parameter WHERE USER_ID=?)";
		...
		try {
			...
			while (rs.next()) {
				...
				/*
				 * if (rs.getString("WECHAT_ID") ==
				 * null||rs.getString("WECHAT_ID").equals(""))
				 * temp.setHasWeChat("否"); else temp.setHasWeChat("是");
				 */
	...
	// 获取里程记录
	public static List<Mileage> getMileage(String did, String stime,
			String etime) {
		...
		// SELECT * FROM mileage WHERE TERMINAL_ID = '1000104010951201' AND
		// (TIME >= '2014-05-07') AND (TIME <= '2014-05-14')

		String sql = "SELECT * FROM mileage WHERE TERMINAL_ID ='" + did
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				mileage = new Mileage(rs.getString("TIME"),
						rs.getString("MILES"));
	...
	public static int getBalanceByNo(String no) {
		...
		String sql = "SELECT BALANCE FROM `user` WHERE ID =(SELECT USER_ID FROM user_parameter WHERE PARAMETER_ID='"
				+ no + "')";
		ResultSet rs = null;
	...
	// 添加购买记录
	public static void addConsume(String user_id, String consume_time,
			int consume_fee, int service_id, String parameter_id) {
		ConnectionPool connPool = null;
		Connection conn = null;
		PreparedStatement ps = null;
		connPool = ConnectionPool.getPool();
	...
	public static Service getService(int SERVICE_ID, String no) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				service = new Service(rs.getInt("STATE"),
	...
	// 改变service设置
	public static void updatServiceState(int state, String sdate, String edate,
			int SERVICE_ID, String ID) {
	...
	// 改变短信數量
	public static void updateSimNum(String no, int num) {
	...
	// 改变平台时间
	public static void updateService(String no, int addyear) {
		...
		String sql = "update service_parameter set ENDTIME=DATE_ADD(ENDTIME,INTERVAL ? YEAR),STATE=1 WHERE PARAMETER_ID=? AND SERVICE_ID=4";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			...
			System.out.println("改变服务失敗：" + ex.getMessage());
		} finally {
			connPool.closeConnection(ps, conn);
		}
	}
	...
	// 改变平台时间
	public static void updateServiceNow(String no, int addyear) {
		...
		String sql = "update service_parameter set ENDTIME=DATE_ADD(CURDATE(),INTERVAL ? YEAR),STATE=1 WHERE PARAMETER_ID=? AND SERVICE_ID=4;";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 改变流量时间
	public static void updateServiceliuliang(String no, int addyear) {
		...
		String sql = "update service_parameter set ENDTIME=DATE_ADD(ENDTIME,INTERVAL ? YEAR),STATE=1 WHERE PARAMETER_ID=? AND SERVICE_ID=6";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	public static void updateSimMadte(String imsi, int addyear) {
		...
		String sql = "update sim set mdate=DATE_ADD(mdate,INTERVAL ? YEAR) WHERE imsi=?;";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 赠送平台时间
	public static void updateServiceGift(String no, int addmounth) {
		...
		String sql = "update service_parameter set ENDTIME=DATE_ADD(ENDTIME,INTERVAL ? MONTH),STATE=1 WHERE PARAMETER_ID=? AND SERVICE_ID=4";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);

			ps.setInt(1, addmounth);
			ps.setString(2, no);
	...
	// 赠送平台时间
	public static void updateServiceGiftNow(String no, int addmounth) {
		...
		String sql = "update service_parameter set ENDTIME=DATE_ADD(CURDATE(),INTERVAL ? MONTH),STATE=1 WHERE PARAMETER_ID=? AND SERVICE_ID=4;";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 获取服务结束时间
	public static String getServiceEndTime(String did) {
		...
		String ENDTIME = null;
		ResultSet rs = null;
		String sql = "SELECT ENDTIME FROM service_parameter WHERE PARAMETER_ID=? AND SERVICE_ID=4;";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 获取gift
	public static String getGiftById(String did) {
	...
	// 设置GIFT信息
	public static void updateGift(String id, String time) {
		...
		String sql = "UPDATE  parameter SET GIFT=? WHERE ID=?";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	public static Boolean add_payrec(String id, String time, String service_id,
			String money, String serial) {
		...
		String sql = "INSERT INTO wechat_pay (id,time,service_id,money,serial) VALUES (?,?,?,?,?)";
		ResultSet rs = null;
		Boolean isTrue = false;
		try {
			conn = connPool.getConnection();
	...
	public static String getImsiById(String id) {
		...
		String ENDTIME = null;
		ResultSet rs = null;
		String sql = "SELECT IMSI FROM terminal WHERE ID=?";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	public static Sim getPhoneByImsi(String imsi) {
		...
		ResultSet rs = null;
		Sim sim = null;
		String sql = "SELECT * FROM sim WHERE imsi=?";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				sim = new Sim(rs.getString("phone"), rs.getString("bsim_id"),
						rs.getString("mdate"),
						rs.getString("flow"));
			}
		} catch (Exception ex) {
	...
	// 购买短信
	/*
	 * public static void updateSimNum(String id,String num) {
	 * 
	 * ConnectionPool connPool = null; Connection conn = null; PreparedStatement
	 * ps = null; connPool = ConnectionPool.getPool();
	 * 
	 * String sql =
	 * "UPDATE `user` SET SIMNUM =SIMNUM+? WHERE ID IN(SELECT USER_ID FROM user_parameter WHERE PARAMETER_ID=?)"
	 * ; try { conn = connPool.getConnection(); ps = conn.prepareStatement(sql);
	 * 
	 * ps.setString(1, num); ps.setString(2, id);
	 * 
	 * ps.executeUpdate(); conn.commit(); } catch (Exception ex) {
	 * System.out.println("改变SIM信息：" + ex.getMessage()); } finally {
	 * connPool.closeConnection(ps, conn); } }
	 */

	// 改变默认选中0
	public static void updateSelect0(String weid) {
		...
		String sql = "UPDATE terminal_wechat SET SELT =0 WHERE WECHAT_ID=?";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 改变默认选中1
	public static void updateSelect1(String id, String weid) {
		...
		String sql = "UPDATE terminal_wechat SET SELT =1 WHERE ID=? AND WECHAT_ID=?;";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 获得默认选中
	public static String getSelect(String weid) {
		...
		String id = null;
		ResultSet rs = null;
		String sql = "SELECT * FROM terminal_wechat WHERE WECHAT_ID=? AND SELT=1";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
	...
	// 获取weid
	public static List<String> getweidByNo(String no) {
		...
		String sql = "SELECT WECHAT_ID from terminal_wechat WHERE ID=?";

		ResultSet rs = null;
		List<String> roundrail = new ArrayList<String>();
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				roundrail.add(rs.getString("WECHAT_ID"));
			}
	...
	public static Boolean UPDATEticket(String device_id, String ticket) {
		...
		// String sql = "INSERT INTO parameter(ID, ticket) SELECT '"+device_id
		// +"', '"+ticket+"' FROM DUAL WHERE NOT EXISTS(SELECT * FROM parameter WHERE ID = '"+device_id+"')";
	...
	public static String getIdByImei(String imei) {
		...
		String sql = "SELECT ID FROM terminal WHERE IMEI=?";
	...
	public static Boolean checkDevice(String device_id) {
	...
	public static String getICCIDById(String id) {
		...
		String sql = "SELECT iccid FROM sim WHERE imsi IN (SELECT IMSI FROM terminal WHERE ID=?)";
	...
	// 改变天气设置
	public static Boolean changeName(String name, String ID) {
		...
		Boolean isSucc = false;
		String sql = "UPDATE parameter SET ALIAS = ? WHERE id=?";
		try {
			conn = connPool.getConnection();
			ps = conn.prepareStatement(sql);
